...
* Check *manually* that the Resource title [Citation Title](#citationtitle) has a unambiguous title. [Manual message](#manualcitationtitle)
  ...
  * GSAA: claim year, the Member stateand if applicable, the region..

**Test method**
* Verify ¿manually? that the [Resource locator](#resourcelocator) metadata pointing to a spatial data service (to a download service WMS, WFS view service, etc). It should not point to a general information web page. Otherwise report [¿manual?Resource locator error message](#resourcelocatorerrormessage).
